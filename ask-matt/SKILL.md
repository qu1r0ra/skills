...
3. **Branch: is this a multi-session build?**
   - **Yes** → **`/to-spec`** (turn the thread into a spec), then **`/to-tickets`** to split it into tracer-bullet tickets, each declaring its **blocking edges**. On a local tracker that's one file per ticket under `.scratch/<feature>/issues/`, worked blockers-first by hand; on a real tracker the edges become native blocking links, so any ticket whose blockers are done can be grabbed: run **`/implement`** on one or more ready tickets as a context-saving implementation batch, then **`/clear`** between batches when useful. Each ticket remains self-contained, even when several are implemented together.
   - **No** → **`/implement`** right here, in the same context window.

   Either way, **/implement** builds the selected issue set by driving **/tdd** internally (one red-green slice at a time), then closes out by running **/code-review**, a two-axis review (Standards + Spec) of the diff. A single `/implement` run may cover multiple independent or jointly-ready tickets. Once verified, run **/sync** once for that implementation batch to reconcile every completed ticket and its Wayfinder decision maps, run lightweight closeout checks, commit only reconciliation changes, and optionally push before deciding on a phase boundary. Reach for **/tdd** on its own when you just want to build a concrete behaviour test-first without a full spec, and **/code-review** on its own whenever you want to review a branch or PR against a fixed point.

### Context hygiene

Keep steps 1–3 in **one unbroken context window** (don't compact or clear until after `/to-tickets`) so the grilling, spec, and tickets all build on the same thinking. Each `/implement` then starts fresh, working from one ticket or a bounded set of tickets; `/sync` closes that whole implementation batch.
...
- **`/sync`**: the end-of-task / end-of-session closeout. It reconciles every completed ticket in the current implementation batch and its Wayfinder decision maps, runs lightweight checks, commits only reconciliation files, optionally pushes, and emits a receipt with the next frontier tickets.
